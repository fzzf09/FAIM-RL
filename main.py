...
parser.add_argument('--model', type=str, default='GCN_MLP_Model', help='model name')
parser.add_argument('--model_file', type=str, default='FAIM.ckpt', help='model file name')
...
def main():
    ...
    path_graphs.sort(key=natural_sort_key)
    print(path_graphs)

    graph_lst = [graph_utils.read_graph(path_g, ind=0, directed=True) for path_g in path_graphs]
    graph_lst_realization=[graph_utils.load_graph(path_g) for path_g in path_graphs]
    ...
    args.T = 3
    args.memory_size = 50000000
    args.reg_hidden = 32
    args.embed_dim = 32

    logging.info(f'Loading agent {args.model}')
    agent = rl_agents.Agent(args)
    agent.get_graph_node_emb(graph_lst_realization,graph_lst)
    print('generate get_graph_node_emb')

    # create environment
    logging.info('Loading environment %s' % args.environment_name)
    train_env = environment.Environment(args.environment_name, graph_lst,graph_lst_realization, args.budget)
    test_env = environment.Environment(args.environment_name, graph_lst,graph_lst_realization, args.budget)
